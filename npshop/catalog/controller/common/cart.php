...
class ControllerCommonCart extends Controller {
	public function index() {
		...
		if ($this->config->get('config_store_id') == 0) {
			$data['text_items'] = sprintf($this->language->get('text_items_noprice'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0));
			unset($data['text_checkout']);
		} else {
